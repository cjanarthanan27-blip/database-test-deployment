...
const Locations = () => {
    ...
    const handleCategoryCountChange = (e) => {
        ...
        setFormData(prev => {
            ...
            // Auto-calculation logic: student_count - exclude_value
            if (name === 'student_count' || name === 'exclude_value') {
                const sCount = name === 'student_count' ? parseInt(value) || 0 : parseInt(prev.student_count) || 0;
                const eValue = name === 'exclude_value' ? parseInt(value) || 0 : parseInt(prev.exclude_value) || 0;
                newData.second_count = Math.max(0, sCount - eValue); // Ensure not negative
            }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (activeTab === 'Borewell' || activeTab === 'Well') {
                ...
                payload.student_count = parseInt(payload.student_count) || 0;
                payload.exclude_value = parseInt(payload.exclude_value) || 0;
                payload.second_count = parseInt(payload.second_count) || 0;
                payload.has_student_count = !!payload.has_student_count;
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                    <thead className="bg-gray-50 dark:bg-slate-900/50">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">
                                {activeTab === 'Category' ? 'Student Count' : 'Address / Description'}
                            </th>
                            {activeTab === 'Category' && (
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">
                                    Second Count
                            ...
                            locations.map((location, index) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">
                                        {activeTab === 'Category' ? location.student_count : (location.address || location.description || '-')}
                                    </td>
                                    {activeTab === 'Category' && (
                                        <td className="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">
                                    ...
                                    {activeTab === 'Category' && (
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${location.has_student_count
                                                ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300'
                                                : 'bg-gray-100 text-gray-800 dark:bg-gray-900/40 dark:text-gray-300'
                                                }`}>
                                                {location.has_student_count ? 'Enabled' : 'Disabled'}
                                            </span>
                                        </td>
                                    )}
                                    {(activeTab === 'Borewell' || activeTab === 'Well') && (
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    type="number"
                                    name="exclude_value"
                                    value={formData.exclude_value || 0}
                                    onChange={handleCategoryCountChange}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                                    Second Count (Auto-calculated: Student Count - Exclude Value)
                                </label>
                                <input
                                    type="number"
                                    name="second_count"
                                    value={formData.second_count || 0}
                                    readOnly
                                    className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-md bg-gray-50 dark:bg-slate-800 dark:text-gray-400 cursor-not-allowed"
                                />
                            </div>
                            ...
                            <div className="flex items-center">
                                <input
                                    ...
                                    checked={formData.has_student_count}
                                    onChange={handleChange}
                                ...
                                <label className="ml-2 block text-sm text-gray-900 dark:text-slate-300">
                                    Show Student/Staff Count on Reports
                                </label>
                            </div>
                        </div>
                    )}
